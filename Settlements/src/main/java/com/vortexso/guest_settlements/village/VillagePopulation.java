...
public record VillagePopulation(int children, Map<Identifier, Integer> professions) {
  public VillagePopulation {
    ...
    for (Map.Entry<Identifier, Integer> entry : professions.entrySet()) {
      Identifier profession = Objects.requireNonNull(entry.getKey(), "profession");

      Integer amount = Objects.requireNonNull(entry.getValue(), "profession amount");

      if (amount < 0) {
        throw new IllegalArgumentException("profession amount must be >= 0");
      }
