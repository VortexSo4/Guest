...
public final class VillageWorldManager {
  ...
  public void handleChunkLoad(ChunkPos chunkPos) {
    for (VillageNode node : nodes.values()) {
      ...
      if (box.intersectingChunks().anyMatch(chunk -> chunk.equals(chunkPos))) {
  ...
  private void activateVillage(VillageNode node) {
    VillagePopulation observed = VillagePopulationScanner.scan(level, node.structureBox());

    long currentDay = currentDay();

    VillageState oldState = node.state();
    ...
    if (oldState == null) {
      ...
      node.updateState(
          new VillageState(
              node.id(), node.center(), currentDay, observed, observed.population(), 0.0));

      return;
    }

    long elapsedDays = currentDay - oldState.day();

    if (elapsedDays <= 0) {
      return;
    }

    VillageDayInput input = buildDayInput(node);

    VillageState newState =
        VillageSimulator.simulateDays(oldState, input, level.getSeed(), PARAMETERS, elapsedDays);

    node.updateState(newState);

    VillageStateRestorer.restore(level, node.structureBox(), newState);
  }

  private void captureUnloadedState(VillageNode node) {
    ...
    if (oldState == null) {
      node.updateState(
          new VillageState(
              node.id(), node.center(), day, population, population.population(), 0.0));
  ...
  private VillageDayInput buildDayInput(VillageNode node) {
    double fieldCapacity = 0.0;

    for (VillageFarmRegion region : node.farmRegions()) {

      fieldCapacity += region.farmlandAmount();
    }

    return new VillageDayInput(fieldCapacity, DEFAULT_FERTILITY, DEFAULT_ZOMBIE_PRESSURE, 0, 0);
  }
  ...
  private void rebuildFarmRegions(VillageNode node, StructureStart start) {
    ...
    for (StructurePiece piece : start.getPieces()) {
      ...
      VillageFarmRegion region = new VillageFarmRegion(node.id(), pieceBox);

      region.update(VillageFarmScanner.scan(level, pieceBox));
    ...
    for (BlockPos offset : NEIGHBOR_PROBES) {
      ...
      BlockPos candidate =
          level.findNearestMapStructure(
              net.minecraft.tags.StructureTags.VILLAGE, query, NEIGHBOR_SEARCH_RADIUS, false);
  ...
  public synchronized VillageDebugSnapshot snapshot() {
    ...
    for (VillageNode node : nodes.values()) {
      ...
      for (VillageFarmRegion region : node.farmRegions()) {
        ...
        farmlandAmount += region.farmlandAmount();
      ...
      villages.add(
          new VillageDebugSnapshot.VillageSnapshot(
              ...
              node.structureBox(),
              farmlandAmount,
              node.state() == null ? null : node.state().villagePopulation(),
              List.copyOf(farms)));
    }
    ...
    for (RoadEdge road : roads) {
      ...
      roadSnapshots.add(
          new VillageDebugSnapshot.RoadSnapshot(
              road.firstVillageId(), road.secondVillageId(), first.center(), second.center()));
    }

    return new VillageDebugSnapshot(List.copyOf(villages), List.copyOf(roadSnapshots));
  }
}
